# This is the image transmission code on the Jetson Nano which does not use the model, this is for testing image transmission by itself and should be modified to fit your needs accordingly
# The Image transmission process involves a handshake message first to notify the packets to expect before proceeding to send all the encapsulated data packets for the base station to assemble the image data
...
def send_image(image, cycle):
    ...
    # In a loop, send the image multiple times to match the rate of the FC data, packets are lost and this is a work around to ensure image transmission consistency
    for s in range(4):
